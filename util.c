...
#define CHECK_LASTNOT(str, suffixc) suffixc != str[strlen(str) - 1]
  va_list argp;
  ...
  if (CHECK_LASTNOT(part, '/')) strlcat(dst_path, "/", PATH_MAX);
  ...
  for (;;) {
    ...
    if (oldp && CHECK_LASTNOT(oldp, '/')) strlcat(dst_path, "/", PATH_MAX);
